export default function PreviewPage() {
  const deities = [
    { name: 'Murugan', tamil: 'முருகன்', desc: 'Dewa Perang & Kebijaksanaan', gradient: 'linear-gradient(135deg, #f59e0b, #d97706)' },
    { name: 'Shiva', tamil: 'சிவன்', desc: 'Dewa Penghancur & Transformasi', gradient: 'linear-gradient(135deg, #8b5cf6, #6d28d9)' },
    { name: 'Shakti', tamil: 'சக்தி', desc: 'Dewi Kekuatan & Energi', gradient: 'linear-gradient(135deg, #ec4899, #be185d)' },
  ]
    ...
    <main style={{
      ...
      <nav style={{
        ...
        <div style={{display: 'flex', gap: 24}}>
          {['Dewa-Dewi', 'Festival', 'Doa', 'Kitab Suci'].map((item, i) => (
            <span key={i} style={{color: 'rgba(252,211,77,0.7)', fontSize: 14, cursor: 'pointer', letterSpacing: 0.5}}>{item}</span>
          ))}
        </div>
      </nav>
      ...
      <div style={{textAlign: 'center', padding: '100px 24px 60px', position: 'relative'}}>
        ...
        <p style={{
          ...
          marginBottom: 50,
          letterSpacing: 1,
        }}>Aplikasi Spiritual Hindu Tamil — Doa · Festival · Kitab Suci</p>
      ...
      <div style={{display: 'flex', gap: 24, padding: '0 32px 80px', flexWrap: 'wrap', justifyContent: 'center'}}>
        {deities.map((d, i) => (
          <div key={i} style={{
            ...
            <p style={{color: '#fb923c', fontSize: 15, marginBottom: 10}}>{d.tamil}</p>
            <p style={{color: 'rgba(156,163,175,0.8)', fontSize: 13, lineHeight: 1.6}}>{d.desc}</p>
          </div>
        ))}
      </div>
